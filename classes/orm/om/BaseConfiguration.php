...
abstract class BaseConfiguration extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the configuration_id field.
	 * @var        int
	 */
	...
	/**
	 * Get the [configuration_id] column value.
	 * 
	 * @return     int
	 */
	...
	/**
	 * Set the value of [configuration_id] column.
	 * 
	 * @param      int $v new value
	 * @return     Configuration The current object (for fluent API support)
	 */
	public function setConfigurationId($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->configuration_id !== $v) {
			$this->configuration_id = $v;
			$this->modifiedColumns[] = ConfigurationPeer::CONFIGURATION_ID;
		}
	...
	public function setDomain($v)
	{
		...
		if ($this->domain !== $v) {
			$this->domain = $v;
			$this->modifiedColumns[] = ConfigurationPeer::DOMAIN;
		}
	...
	public function setKeyName($v)
	{
		...
		if ($this->key_name !== $v) {
			$this->key_name = $v;
			$this->modifiedColumns[] = ConfigurationPeer::KEY_NAME;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->configuration_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->domain = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->key_name = ($row[$startcol + 2] !== null) ? (string) $row[$startcol + 2] : null;
			...
			return $startcol + 4; // 4 = ConfigurationPeer::NUM_COLUMNS - ConfigurationPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					$this->setConfigurationId($pk);  //[IMV] update autoincrement primary key
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ConfigurationPeer::CONFIGURATION_ID)) $criteria->add(ConfigurationPeer::CONFIGURATION_ID, $this->configuration_id);
		if ($this->isColumnModified(ConfigurationPeer::DOMAIN)) $criteria->add(ConfigurationPeer::DOMAIN, $this->domain);
		if ($this->isColumnModified(ConfigurationPeer::KEY_NAME)) $criteria->add(ConfigurationPeer::KEY_NAME, $this->key_name);
	...
	public function buildPkeyCriteria()
	{
		...
		$criteria->add(ConfigurationPeer::CONFIGURATION_ID, $this->configuration_id);
	...
	/**
	 * Generic method to set the primary key (configuration_id column).
	 *
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setConfigurationId(NULL); // this is a auto-increment column, so set to default value
