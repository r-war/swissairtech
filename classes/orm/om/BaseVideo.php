...
abstract class BaseVideo extends BaseObject  implements Persistent {
	...
	/**
	 * The value for the date_closed field.
	 * @var        string
	 */
	...
	/**
	 * Get the [optionally formatted] temporal [date_closed] column value.
	 * 
	 * This accessor only only work with unix epoch dates.  Consider enabling the propel.useDateTimeClass
	 * option in order to avoid converstions to integers (which are limited in the dates they can express).
	 *
	 ...
	 * @throws     PropelException - if unable to parse/validate the date/time value.
	 */
	public function getDateClosed($format = 'Y-m-d H:i:s')
	{
		...
		if ($this->date_closed === '0000-00-00 00:00:00') {
			// while technically this is not a default value of NULL,
			// this seems to be closest in meaning.
			return null;
		} else {
			try {
				$dt = new DateTime($this->date_closed);
			} catch (Exception $x) {
				throw new PropelException("Internally stored date/time/timestamp value could not be converted to DateTime: " . var_export($this->date_closed, true), $x);
			}
		}
	...
	public function setLanguage($v)
	{
		...
		if ($this->language !== $v) {
			$this->language = $v;
			$this->modifiedColumns[] = VideoPeer::LANGUAGE;
		}
	...
	public function setDate($v)
	{
		...
		if ( $this->date !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = VideoPeer::DATE;
			}
	...
	/**
	 * Sets the value of [date_closed] column to a normalized version of the date/time value specified.
	 * 
	 * @param      mixed $v string, integer (timestamp), or DateTime value.  Empty string will
	 *						be treated as NULL for temporal objects.
	 * @return     Video The current object (for fluent API support)
	...
	public function setDateClosed($v)
	{
		...
		if ( $this->date_closed !== null || $dt !== null ) {
			// (nested ifs are a little easier to read in this case)

			$currNorm = ($this->date_closed !== null && $tmpDt = new DateTime($this->date_closed)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->date_closed = ($dt ? $dt->format('Y-m-d H:i:s') : null);
				$this->modifiedColumns[] = VideoPeer::DATE_CLOSED;
			}
		} // if either are not null
	...
	public function setCode($v)
	{
		...
		if ($this->code !== $v) {
			$this->code = $v;
			$this->modifiedColumns[] = VideoPeer::CODE;
		}
	...
	public function setPicture($v)
	{
		...
		if ($this->picture !== $v) {
			$this->picture = $v;
			$this->modifiedColumns[] = VideoPeer::PICTURE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->date_closed = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(VideoPeer::LANGUAGE)) $criteria->add(VideoPeer::LANGUAGE, $this->language);
		if ($this->isColumnModified(VideoPeer::DATE)) $criteria->add(VideoPeer::DATE, $this->date);
		if ($this->isColumnModified(VideoPeer::DATE_CLOSED)) $criteria->add(VideoPeer::DATE_CLOSED, $this->date_closed);
		if ($this->isColumnModified(VideoPeer::CODE)) $criteria->add(VideoPeer::CODE, $this->code);
		if ($this->isColumnModified(VideoPeer::NAME)) $criteria->add(VideoPeer::NAME, $this->name);
		if ($this->isColumnModified(VideoPeer::PICTURE)) $criteria->add(VideoPeer::PICTURE, $this->picture);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setDateClosed($this->date_closed);
